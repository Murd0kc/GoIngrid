...
const out = [
  ...
  "do $$ declare level_id uuid; module_id uuid; topic_id uuid; lesson_id uuid; exercise_id uuid; begin",
  "select id into level_id from public.levels where code='A1';",
]
...
for (const name of files) {
  ...
  if (moduleCode !== currentModule) {
    out.push(`select m.id into module_id from public.modules m where m.level_id=level_id and m.sort_order=${moduleOrder};`)
    out.push(`if module_id is null then insert into public.modules(level_id,title,description,sort_order) values(level_id,${sql(moduleTitle)},${sql('Contenido progresivo de nivel A1')},${moduleOrder}) returning id into module_id; end if;`)
    currentModule = moduleCode
    currentTopic = ''
  }
  if (topicCode !== currentTopic) {
    out.push(`select t.id into topic_id from public.topics t where t.module_id=module_id and t.sort_order=${topicOrder};`)
    out.push(`if topic_id is null then insert into public.topics(module_id,title,description,sort_order) values(module_id,${sql(topicTitle)},${sql(lesson.communication_goal)},${topicOrder}) returning id into topic_id; end if;`)
    currentTopic = topicCode
  }
  out.push(`select l.id into lesson_id from public.lessons l where l.topic_id=topic_id and l.sort_order=${lessonOrder};`)
  out.push(`if lesson_id is null then insert into public.lessons(topic_id,title,objective,estimated_minutes,estimated_seconds,is_published,sort_order,skill_focus,cefr_objectives) values(topic_id,${sql(lesson.title)},${sql(lesson.communication_goal)},${Math.max(1, Math.ceil(lesson.estimated_seconds/60))},${lesson.estimated_seconds},true,${lessonOrder},ARRAY[${sql(lesson.pronunciation?.targets ? 'pronunciation' : 'integrated')}],ARRAY[${sql('A1 objective')}]) returning id into lesson_id; else update public.lessons set title=${sql(lesson.title)},objective=${sql(lesson.communication_goal)},estimated_seconds=${lesson.estimated_seconds},estimated_minutes=${Math.max(1, Math.ceil(lesson.estimated_seconds/60))},is_published=true where id=lesson_id; end if;`)
  lesson.activities.forEach((activity, index) => {
    const correct = activity.evaluation?.accepted_answers ?? []
    out.push(`insert into public.exercises(lesson_id,exercise_type,skill,prompt,explanation,correct_answer,estimated_seconds,difficulty,sort_order,is_published) values(lesson_id,${sql(activity.type)},${sql(activity.skill)},${sql(activity.prompt ?? activity.instruction)},${sql(activity.feedback_correct ?? activity.feedback ?? '')},${json(correct)},${activity.estimated_seconds},${Math.min(5, Math.max(1, Number(activity.difficulty) || 1))},${index + 1},true) on conflict(lesson_id,sort_order) do update set exercise_type=excluded.exercise_type,skill=excluded.skill,prompt=excluded.prompt,explanation=excluded.explanation,correct_answer=excluded.correct_answer,estimated_seconds=excluded.estimated_seconds,is_published=true returning id into exercise_id;`)
    const options = Array.isArray(activity.options) ? activity.options : []
    if (options.length > 0) {
      out.push('delete from public.exercise_options where exercise_id=exercise_id;')
      options.forEach((option, optionIndex) => {
        ...
        out.push(`insert into public.exercise_options(exercise_id,option_text,is_correct,sort_order) values(exercise_id,${sql(text)},${isCorrect},${optionIndex + 1});`)
      })
    }
  })
}
